...
export default class MenuBar extends React.PureComponent<MenuBarProps, MenuBarState> {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (doc) {
        this.setState({ isShowProfilePanel: true }, () => {
          const firstProfile = doc.getElementById(this.props.profiles[0].id);
          if (firstProfile) {
            firstProfile.focus();
          }
        });
  ...
  handleProfileItemonKeyUp = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    ...
    const doc = e.currentTarget.ownerDocument;
    if (doc) {
      const nextProfile = doc.getElementById(profiles[nextID].id);
      if (nextProfile) {
        nextProfile.focus();
      }
    }
  ...
  componentDidMount() {
    this.suggestVisibility$.pipe(debounceTime(100)).subscribe(flag => {
      this.setState({ isShowSuggestPanel: flag });
    });

    this.suggestsRequest$
      .pipe(
  ...
  renderSuggestsPanel = () => {
    return (
      <div
        className="panel-MenuBar_SuggestsWrapper"
        style={{
        ...
        <ul className="panel-MenuBar_Suggests">
          {this.state.suggests.map(s => (
            <li key={s.entry} className="panel-MenuBar_SuggestsItem">
              <button
                className="panel-MenuBar_SuggestsBtn"
  ...
  renderProfilePanel = () => {
    const { t, profiles, activeConfigID } = this.props;
    return (
      <ul
  ...
  render() {
    ...
    const { isShowProfilePanel, isShowSuggestPanel } = this.state;
    ...
    return (
      <header className="panel-MenuBar">
        ...
        <div
          className="panel-MenuBar_DragArea"
