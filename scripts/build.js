  ...
  return Promise.all(
    browsers.map(browser => {
      ...
      return Promise.all([
        ...
        // project files
        ...files.map(file =>
          fs.copy(file.path, path.join(dest, file.name), {
            dereference: true,
            // remove js files for css only chunks
            filter: file =>
              !/[\\\/]+(panel(-internal)?|dicts[\\\/]+[^\\\/]+)\.js(\.map)?$/.test(
                file
              )
          })
        )
...
function writeLocales(localesPath, localesJSON) {
  ...
  return fs.mkdir(localesPath).then(() =>
    Promise.all(
      locales.map(lang => {
        ...
        return fs
          ...
          .then(() =>
            fs.writeFile(
              path.join(langPath, "messages.json"),
              JSON.stringify(localesJSON[lang], null, "  ")
            )
          );
      })
    )
...
function patchInternalCSS() {
  const styles = fs
    .readdirSync(path.join(paths.appBuild, "dicts"))
